$(() => {
  ...
  const add_cards_target = $('.add-cards');
  let additional_classes;
  const template =
    '<section class="card {{additional_classes}}"><div class="inner"><p>{{content}}</p></div></section>';

  function random_array_item(v, prev) {
    if (typeof prev !== 'undefined') {
      const prev_item_index = v.indexOf(prev);
  ...
  function generate_new_cards() {
    ...
    while (i < 4) {
      ...
      var stunt;
      var loop_template;
      const high_or_low = Math.round(Math.random()) ? '0' : '1';
      ...
      var loop_template = template
        .replace('{{content}}', stunt)
        .replace('{{additional_classes}}', additional_classes);
